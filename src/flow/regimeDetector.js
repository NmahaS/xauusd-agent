...
export function detectRegime(h1Candles, h4Candles, indicators) {
  ...
  if (volatilityRatio > 8.0) {
    // Only block at EXTREME volatility (flash crash / news spike)
    regime = 'volatile';
    confidence = 85;
    smc_effective = false;
    reasoning = `Extreme volatility ${volatilityRatio.toFixed(1)}x — avoid trading`;
  } else if (volatilityRatio > 3.0) {
    // Elevated but tradeable — normal for Monday open and weekend gaps
    regime = 'volatile_tradeable';
    confidence = 70;
    smc_effective = true;
    reasoning = `Elevated volatility ${volatilityRatio.toFixed(1)}x — trade with tight stops`;
  ...
  } else if (rangeToATR < 2.5) {
    // Only block SMC when EXTREMELY tight range
    regime = 'ranging';
    confidence = 75;
    smc_effective = false;
    reasoning = `Extremely tight range (${rangeToATR.toFixed(1)}×ATR) — SMC signals unreliable`;
  } else if (rangeToATR < 4) {
    // Moderate range — allow trading but with caution
    regime = 'ranging_tradeable';
    confidence = 65;
    smc_effective = true;
    reasoning = `Moderate range (${rangeToATR.toFixed(1)}×ATR) — trade with caution`;
  } else {
