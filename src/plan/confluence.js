...
export function computeConfluence(ctx) {
  ...
  const h4Bias = ctx.h4?.structure?.bias;
  const m15Bias = ctx.m15?.structure?.bias;
  const h1Bias = ctx.h1?.structure?.bias;
  ...
  const currentPrice = ctx.m15?.indicators?.lastClose ?? ctx.h1?.indicators?.lastClose ?? ctx.currentPrice;
  ...
  const m15OBs = direction === 'long'
    ? (ctx.m15?.orderBlocks?.bullish || [])
    : (ctx.m15?.orderBlocks?.bearish || []);
  ...
  const m15FVGsAll = direction === 'long'
    ? (ctx.m15?.fvgs?.bullish || [])
    : (ctx.m15?.fvgs?.bearish || []);
  const m15FVGs = m15FVGsAll.filter(f => !f.filled);
  ...
  const m15Struct = ctx.m15?.structure;
  ...
  const h1OBs = direction === 'long'
    ? (ctx.h1?.orderBlocks?.bullish || [])
    : (ctx.h1?.orderBlocks?.bearish || []);
  ...
  const m15Div = ctx.m15?.indicators?.divergence;
  const h1Div = ctx.h1?.indicators?.divergence;
  ...
  // Check 13: H1 bias agrees with H4
  if (h1Bias === h4Bias && h4Bias !== 'ranging') {
    factors.push(`H1 bias agrees with H4 (${h4Bias})`);
    out.count++;
  }

  // Check 14: Price vs weekly VWAP confirms direction
  if (ctx.vwap?.weeklyVWAP) {
    if (direction === 'short' && ctx.currentPrice < ctx.vwap.weeklyVWAP) {
      factors.push('Below weekly VWAP — institutional bearish');
      out.count++;
    } else if (direction === 'long' && ctx.currentPrice > ctx.vwap.weeklyVWAP) {
      factors.push('Above weekly VWAP — institutional bullish');
      out.count++;
    }
  }

  // Check 15: Full LLM consensus
  if (ctx.consensus?.agreement === 'full') {
    factors.push('Full LLM consensus (Claude + DeepSeek agree)');
    out.count++;
  }

  // Check 16: Funding rate contrarian confirms direction
  if (ctx.funding?.rate > 0.0001 && direction === 'short') {
    factors.push('Funding positive — longs paying (contrarian bearish)');
    out.count++;
  } else if (ctx.funding?.rate < -0.0001 && direction === 'long') {
    factors.push('Funding negative — shorts paying (contrarian bullish)');
    out.count++;
  }
