...
export async function updateDailySummary(timestampIso, { save = true } = {}) {
  ...
  try {
    files = (await fs.readdir(dir)).filter(f =>
      f.endsWith('.json') && f !== 'daily-summary.json' && f !== 'trades.json'
    );
  } catch {
  ...
  // Only plans where execution actually happened on exchange count as "trades"
  const executedPlans = plans.filter(p => p.execution?.executed === true);
  const signalPlans = plans.filter(p => p.direction != null && p.execution?.executed !== true);
  const noTradePlans = plans.filter(p => p.direction == null);
  const allDirectionalPlans = [...executedPlans, ...signalPlans];
  ...
  // Direction counts (executed + signals — useful to see overall directional bias)
  const directions = { long: 0, short: 0 };
  for (const p of allDirectionalPlans) {
    if (p.direction === 'long') directions.long++;
    else if (p.direction === 'short') directions.short++;
  }

  // Outcome groupings — ONLY executed plans have real outcomes
  const withOutcome = executedPlans.filter(p => p.outcome && p.outcome.status !== 'open');
  ...
  const openTrades = executedPlans.filter(p => !p.outcome || p.outcome.status === 'open');
  const resolvedCount = wins.length + partialWins.length + losses.length;

  // Win stats by quality (executed only)
  const winsByQuality = {};
  for (const q of ['A+', 'A', 'B']) {
    const qt = executedPlans.filter(p => p.setupQuality === q);
  ...
  // Win stats by session (executed only)
  const sessionNames = [...new Set(executedPlans.map(p => p.session?.current).filter(Boolean))];
  const winsBySession = {};
  for (const sess of sessionNames) {
    const st = executedPlans.filter(p => p.session?.current === sess);
  ...
  // Per-plan summaries (executed plans only)
  const planSummaries = executedPlans.map(p => {
  ...
  const totalPotRR1 = r(sumRR(executedPlans, 0));
  const totalPotRR2 = r(sumRR(executedPlans, 1));
  const totalPotRR3 = r(sumRR(executedPlans, 2));
  const n = executedPlans.length;

  const actualRRs = withOutcome.map(p => p.outcome.actualRR ?? 0);
  const totalActualRR = r(actualRRs.reduce((s, v) => s + v, 0));
  const netRR = totalActualRR;

  // Best = highest maxRR potential (or actualRR if resolved)
  const bestPlan = executedPlans.length
    ? executedPlans.reduce((best, p) => {
  ...
  const summary = {
    ...
    trades: executedPlans.length,
    signals: signalPlans.length,
  ...
  if (save) {
    ...
    console.log(
      `[tracker] ${dateStr}: ${plans.length} plans, ${executedPlans.length} executed, ` +
      `${signalPlans.length} signals, ${wins.length + partialWins.length}W ${losses.length}L ${openTrades.length} open`
    );
  }
