...
function planDatePath(timestampIso) {
  ...
  return { dir: path.join(PLANS_DIR, `${y}-${m}-${day}`), filename: `${hour}-${minute}.json` };
}
...
export async function savePlan(plan) {
  ...
  console.log(`[writer] ✅ saved: ${fullPath}`);
  console.log(`[writer] file exists check: ${existsSync(fullPath)}`);
