...
│   ├── memory/
│   │   ├── tradeDB.js        # SQLite trade database (RAG)
│   │   ├── similarity.js     # Similar trade search + WR/EV analysis
│   │   └── context.js        # Historical context builder for LLM
│   ├── plan/
...
│   │   ├── outcomeTracker.js # Win/loss resolution + RAG DB sync
│   │   ├── confluence.js     # Deterministic 12-point confluence scorer
│   │   ├── weeklyGoal.js     # Weekly goal tracking + spot→perp transfer
│   │   ├── monthlyReport.js  # Monthly performance report builder
│   │   ├── yearlyReport.js   # Monthly/yearly performance reports
...
│   │   └── commands.js       # All command handlers (23 commands)
│   └── utils/
...
├── data/
│   └── trades.db             # SQLite RAG trade memory (Railway persistent)
├── scripts/
│   ├── safe-push.sh          # Conflict-safe push (handles bot commits)
│   ├── setup-railway-webhook.js
│   ├── smoke-executor.js     # Executor smoke test
│   ├── importHistoricalTrades.js  # Import HL fills to RAG DB
│   └── testTransfer.js            # Test spot-to-perp transfer
├── .github/workflows/
...
### Every 15 minutes — single full run
1. `resolveOpenTrades()` + `resolveBlockedSignals()` — update RAG DB outcomes
2. `fetchAllHLData()` — H1/H4/M15 candles + mark price + funding + EUR/USD proxy
3. `fetchFredMacro()` + `fetchSentiment()` + `fetchCalendar()` — in parallel
4. `computeClassicalIndicators()` on H1/H4/M15
5. `smcForTimeframe()` on H1/H4/M15 (structure + OBs + FVGs + liquidity + P/D)
6. `buildHistoricalContext()` — RAG context from similar past trades
7. `runLLMConsensus()` — Claude + DeepSeek in parallel (RAG context injected) + optional Perplexity
8. `computeThreeLayerConsensus()` — weekly macro + flow + technical → tier
9. `executeIfApproved()` — market order immediately if AUTO_TRADE=true and all gates pass
10. `saveTrade()` — persist to RAG DB (executed or blocked with reason)
11. `savePlan()` + `formatPlanForTelegram()` + `notify()`
...
- TP1 RR < 1.5
- Confluence < 5
- ATR < 3 pts (dynamic market quality check)
- Spread > 3 pts (dynamic market quality check)
- Funding rate extreme (dynamic market quality check)
