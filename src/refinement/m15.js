...
const PROXIMITY_PTS = 15;         // "in zone" tolerance in price units — gold moves ~$5-20/candle
const EXTENDED_PROXIMITY_PTS = 30; // extended zone: M15 bias market entry fallback
const ATR_PERIOD = 14;
...
export function refineEntry(plan, m15Candles, currentPrice) {
  ...
  if (!inZone(price, [zLo, zHi])) {
    // Extended zone fallback: if M15 bias confirms direction and price is within
    // EXTENDED_PROXIMITY_PTS, allow a market entry rather than waiting indefinitely
    const struct = analyzeStructure(m15Candles, 3);
    const inExtended = price >= zLo - EXTENDED_PROXIMITY_PTS && price <= zHi + EXTENDED_PROXIMITY_PTS;

    const targetBias = plan.direction === 'long' ? 'bullish' : 'bearish';
    const m15SupportsDirection =
      struct?.bias === targetBias ||
      (struct?.lastEvent === 'CHoCH' && struct?.bias === targetBias) ||
      (struct?.lastEvent === 'BOS' && struct?.bias === targetBias);

    if (inExtended && m15SupportsDirection) {
      ...
      const refinedPlan = recomputePlanWithRefinedEntry(plan, price, refinedSL, 'extended-zone');
      if (refinedPlan) {
        console.log(`[m15] CONFIRMED_EXTENDED: price ${price.toFixed(2)} within ${EXTENDED_PROXIMITY_PTS}pts of POI`);
        return {
          refined: true,
          status: 'CONFIRMED_EXTENDED',
          reason: `Extended zone market entry at ${price.toFixed(2)} — M15 ${struct.bias} bias confirms`,
          plan: refinedPlan,
        };
      }
    }
