...
// computeConfluence expects { bullish: [], bearish: [] } but SMC modules return flat arrays.
function splitByType(zones) {
  const arr = Array.isArray(zones) ? zones : [];
  return {
    bullish: arr.filter(z => z.type === 'bullish'),
    bearish: arr.filter(z => z.type === 'bearish'),
  };
}
...
export async function runBacktest(h1Candles, h4Candles) {
  ...
  for (let i = start; i < end; i++) {
    ...
    // computeConfluence expects ctx.h4 / ctx.m15 / ctx.h1 shape.
    // Backtest has no M15 candles — use H1 as the signal TF (closest proxy).
    // splitByType converts flat SMC arrays → { bullish, bearish } as confluence expects.
    const ctx = {
      h4: {
      ...
      session,
      currentPrice: h1Indicators.lastClose,
    };
